...
/**
 * Test script using npx react-context-mcp@latest
 */
...
async function main() {
  const targetUrl = process.env.TARGET_URL || 'http://localhost:57267/dashboard';

  console.error(`Testing npx react-context-mcp@latest with: ${targetUrl}`);
  console.error('='.repeat(60));

  // Spawn the MCP server via npx
  const mcpServer = spawn('npx', ['-y', 'react-context-mcp@latest', '--isolated', '--headless'], {
  ...
  // Wait for npx download and server start
